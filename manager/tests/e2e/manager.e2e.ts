...
// One admin, one database: the steps build on each other.
test.describe.configure({ mode: 'serial' });

let shareUrl = '';
let personUrl = '';

test('first-run setup creates an admin and signs in', async ({ page }) => {
	await setupAdmin(page);
	await expect(page.getByRole('heading', { name: 'Health' })).toBeVisible();
	// Setup is closed once an admin exists.
	await page.goto('/setup');
	await expect(page).not.toHaveURL(/\/setup$/);
});

test('sign out and log in again with password and TOTP', async ({ page }) => {
	await login(page);
...
test('create a person from the People page', async ({ page }) => {
	await login(page);
...
test('bind a template, then push it to AIOStreams', async ({ page }) => {
	await login(page);
	...
	await streams.getByRole('button', { name: 'Push' }).click();
	await expect(page.getByText('Push to AIOStreams queued')).toBeVisible();
	// The job runs in the background; the page refreshes when it finishes.
	await expect(async () => {
		await page.reload();
		await expect(page.locator('#binding-aiostreams').getByText('In sync')).toBeVisible({
			timeout: 2_000
		});
...
test('create a share link and open the public share page', async ({ page, browser }) => {
	await login(page);
	await page.goto(personUrl);
	...
	const install = pub.getByRole('link', { name: 'Install in Stremio' });
	await expect(install).toHaveAttribute('href', /^stremio:\/\//);
	await expect(pub.getByRole('button', { name: 'Copy manifest URL' })).toBeVisible();
	await expect(pub.locator('nav, aside')).toHaveCount(0);

	await pub.goto('/s/not-a-real-token-000000000000000');
	await expect(pub.getByRole('heading', { name: 'This link has expired or is not valid' })).toBeVisible();
	await anon.close();
});

test('revoking the share link turns the page into the expired page', async ({ page, browser }) => {
	await login(page);
	await page.goto(personUrl);
